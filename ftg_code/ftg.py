...
class LidarProcessingNode(Node):
    ...
    def lidar_callback(self,msg):
        angles=self.angle_assignment(msg)
        smoothened_array,length=self.preprocess(msg,angles)
        ...
        ackermann_msg.drive.steering_angle = -1*(135-phi_gap)*(np.pi/180)  
        ackermann_msg.drive.steering_angle_velocity = turn_vel  
        ackermann_msg.drive.speed = vel  
        self.ackermann_publisher.publish(ackermann_msg)

        self.get_logger().info(f"Phi Gap: {135-phi_gap}, Turn Vel: {turn_vel}, Velocity: {vel}")
    ...
    def preprocess(self, msg,angles):
        ...
        smoothened_array=smoothened_array[::20]
        length=len(smoothened_array)
        return smoothened_array,length
    ...
    def parity(self, new_array):
        ...
        threshold = 0.5 # Used to set a particular threshold for discontinuity identification
    ...
    def heading_angle(self, phi1, phi2, new_array, phi_index):
        ...
        phi_gap = np.degrees(
            np.arccos((d1+(d2*(np.cos(np.radians(phi2-phi1)))))/denominator))+phi1-45
            """
        phi_gap=(phi1+phi2)/2
        return phi_gap
    
    def throttle(self, phi_gap):
        vel = 0.1+vel_max*(1-math.exp(-1*alpha*(phi_gap*np.pi/180)))
        return vel
    
    def turn(self, phi_gap):
        turn_vel =1+ k*(phi_gap*np.pi/180)
        return turn_vel
